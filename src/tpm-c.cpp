...
tpm_error_t tpm_context_init(tpm_object_t* context) {
    ...
    try {
        *reinterpret_cast<tpm::TPMContext*>(context->context.context) = tpm::TPMContext();
...
tpm_error_t tpm_context_message(const tpm_object_t* context, uint8_t* msg, uint32_t msg_length, tpm_object* response) {
    ...
    const auto* tpm_context = reinterpret_cast<tpm::TPMContext*>(context->context.context);
    auto message = std::vector<uint8_t>(msg, msg + msg_length);
